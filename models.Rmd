...
# still bad
```
```{r model 2 -- top code + transform dep}
set.seed(100) # for reproducing results
...
# still bad.
```
```{r model 2 -- top code + transformation + weights}
set.seed(100) # for reproducing results
...
fixed <- lm(k_mean ~ par_mean + factor(tier) + factor(cohort) + par_mean*factor(tier), data = training, weights = count)
